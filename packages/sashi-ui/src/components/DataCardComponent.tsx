...
export function DataCardComponent({
  ...
  const getIcon = (key: string, value: any) => {
    ...
    const IconComponent =
      ...
      (typeof value === 'number' ? Hash : value instanceof Date ? Clock : Info);
  ...
  const renderDataItem = ([key, value]: [string, any], index: number) => {
    ...
    return (
      <motion.div
        ...
        className="flex items-center p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200"
      >
        {getIcon(key, value)}
