...
import { z } from "zod";
import { getFunctionAttributes, getFunctionRegistry, VisualizationFunction, VisualizationType } from "./ai-function-loader";

const VisualizationDataSchema = z.object({
    toolName: z.string(),
    data: z.record(z.unknown()), // This allows for any key-value pairs in the data object
  });
  
  const VisualizationOutputSchema = z.array(VisualizationDataSchema);
  
const VisualizationFunctionSchema = z.object({
    getName: z.function().returns(z.string()),
    getDescription: z.function().returns(z.string()),
    getVisualizationType: z.function().returns(z.enum(["table", "dataCard"])),
  });

  const LLMVisualizationOutputSchema = z.object({
    visualizations: VisualizationOutputSchema,
  });
export class AIBot {
    ...
    getUXSystemPrompt = (vis_tools: VisualizationFunction[]) => {
...
Finally, if you decide that a component should be generated, you will output the name of the component between <component></component> tags.`
    ...
    setupComponent = async ({
     ...
     const component = getFunctionRegistry().get(componentName) as VisualizationFunction
     ...
     const result = await this.openai.chat.completions.create({
        messages: [{role: "system", content: generateComponentPrompt}, ...messages],
        model,
        temperature,
        max_tokens,
     ...
     const tool_calls = result.choices[0]?.message.tool_calls?.map((tool ) => {
        return {
    ...
    shouldShowVisualization = async ({
        ...
        const viz_messages = [{role: "system", content: system_prompt},{
            role: "user",
        ...
        const result = await this.openai.chat.completions.create({messages: viz_messages, model, temperature, max_tokens})
